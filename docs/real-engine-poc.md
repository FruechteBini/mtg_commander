...
The multi-prompt spell proof, `PROTO-005` DTO/parser alignment, `ARCH-001` architecture decision, `APP-001` workspace and reusable `ENGINE-001` relay client are complete. The browser connects only to the own API layer; the Relay and Forge node remain internal services. The next engineering step is `UI-001`, which normalizes a real `gameView` for the browser UI.

For a short connectivity check without starting a game, run `npm run engine:smoke`. It authenticates with the reusable client and verifies that the configured room appears in the relay room list.
